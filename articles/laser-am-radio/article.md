...
<video class="article-inline-video" controls preload="auto" data-audio-gain="4" aria-label="Laser radio demonstration video">
  <source src="assets/laser-radio/demovid.mp4" type="video/mp4">
  Your browser does not support embedded video.
</video>

Here’s a video of when I first got the system to work. When I cover the beam, the music cuts out, and it plays again when I move my hand out of the way. The beam’s brightness looks completely constant even though there’s actually music modulated into its intensity—you just can’t see it because the carrier is cycling 8 million times per second!
...
<div class="article-text-image article-stage-layout">
  <div class="stage-main">
    <div class="text">
      <p>To start, I generate an 8 MHz carrier wave with a crystal oscillator circuit. I used this <a href="https://youtube.com/watch?v=2omcrkrrhoc">great design by KF5OBS</a>. I then mix this carrier with the input audio signal using a 2N2222 transistor. This varies the amplitude of the carrier according to the audio waveform, which is exactly how AM radio stations produce their signal.</p>
    </div>
...
<figure class="article-diagram article-diagram-compact">
  <img src="assets/laser-radio/scope8MHz.png" alt="Oscilloscope showing nearly perfect 8MHz waveform">
  <figcaption>Oscilloscope showing nearly perfect 8MHz waveform</figcaption>
</figure>
...
<div class="article-text-image article-stage-layout">
  <div class="stage-main">
    ...
    <p>In this configuration, we now have a properly biased laser with the AM wave modulated into the intensity of the beam.</p>
    </div>
    <figure class="article-diagram article-stage-schematic">
...
<div class="article-text-image article-stage-layout">
  <div class="stage-main">
    <div class="text">
    <p>With the signal still modulated, I needed to demodulate it and recover the audio. I did this using a traditional diode-RC envelope detector. Old AM and crystal radios used a germanium diode to rectify the signal, then a capacitor and resistor to create a time constant that follows the envelope of the carrier, leaving the original audio signal.</p>
    <p>I then used a dual-stage TL072 audio preamplifier and fed that into an LM386 to drive a speaker.</p>
    </div>
    <figure class="article-diagram article-stage-schematic">
...
I believe this system could have a good transmission range as long as there isn’t too much interference in the air and the laser can be aligned precisely with the receiver.
...
## Sharing It at OC Maker Faire

I was thrilled to show off my project at the OC Maker Faire on September 12–13. It was a busy weekend for me, with the SAT on Saturday morning. Hundreds of curious people came by my booth, ranging from ham radio operators to company CEOs to makers like myself. I was excited to explain how my project worked. It was really fun to present something I had spent weeks working on to others.
...
Free-space optical communication systems like this are already used for communication between satellites and can offer extremely high bandwidth compared with traditional radio systems. You can read more about [free-space optical communication here](https://en.wikipedia.org/wiki/Free-space_optical_communication).

FSO can also be useful for secure communications because the narrow, line-of-sight beam can be much harder to intercept remotely than a radio signal, although it isn’t impossible. The signal can also be encrypted and can use wavelengths that are invisible to the naked eye.
...
The biggest issue is that aligning the laser beam with the photodiode is extremely difficult, and it gets even harder the farther away the receiver is. The distance could be extended with better collection optics, more receiver gain, or a more powerful laser that allows for greater modulation depth. All of these options have their own caveats regarding safety, price, and complexity, although they are completely feasible.

#### What are some limitations of this project?

Free-space optical communication has several limitations. Environmental factors can limit the maximum distance of the beam, including heat, humidity, rain, snow, fog, animals like birds, and basically anything else that can block, scatter, or refract the beam.

Aligning the beam is also difficult, especially as the transmitter and receiver get farther apart. Over very long terrestrial distances, the curvature of the Earth eventually becomes another limitation to a direct line-of-sight link unless the transmitter and receiver are elevated.
